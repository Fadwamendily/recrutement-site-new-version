...
module.exports = {
    ...
    createcondidat: async (req, res) => {
        try {
            ...
            req.body["cv"] = cvv
            const autreCV = req.files.autreCV[0].filename; // importer deux fichiers en mongoose
    ...
    updatecondidatById: (req, res) => {
        condidatModel.findOneAndUpdate({ _id: req.params.id }, req.body, { new: true }, (err, condidat) => {
            if (err) {
                res.status(500).json({
