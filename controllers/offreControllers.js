...
module.exports = {
    ...
    getoffreById: function (req, res) {
        offreModel.findById({ _id: req.params.id }).populate('userId')
            .exec((err, offre) => {
                if (err) {
    ...
    getoffreByUser: function (req, res) {
        offreModel.find({ userId: req.params.userId })
            .exec((err, offre) => {
                if (err) {
                    res.json({ message: 'error get one offre' + err, data: null, status: 500 })
                }
    ...
    deleteoffreByUser: function (req, res) {
        offreModel.findAndDelete( {userId: req.params.userId})
            .exec((err, offre) => {
    ...
    updateoffreById: (req, res) => {
        offreModel.findOneAndUpdate({ _id: req.params.id }, req.body, { new: true }, (err, offre) => {
            if (err) {
                res.status(500).json({
